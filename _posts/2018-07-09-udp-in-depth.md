...
QUIC ([quic-transport][16]) is a new transport protocol under development/standardization. The goal is to provide a reliable, encrypted, in-sequence delivery, stream-based channel. This protocol aggregates the benefit from several other protocols, such as HTTP2 ([RFC7540][17]), SCTP, and TLS. QUIC runs on top of UDP to reduce the burden of deploying protocol in the middlebox (which is the lesson learned from the IPv6 deployment). It is also implemented in userspace for easy use/update by application, without waiting for the upgrade from operating system provider (lesson learned from TCP new feature deployment).

### features:
-   __reliable connection setup/shutdown__:
